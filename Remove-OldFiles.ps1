<#
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath H:\scripts -FileAge 100 -ListOnly -LogFile H:\log.log
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath H:\scripts -FileAge 30 -LogFile H:\log.log -VerboseLog
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath C:\docs -FileAge 30 -LogFile H:\log.log -ExcludePath "C:\docs\finance\","C:\docs\hr\"

      Description:
      Searches through the C:\docs folder and deletes files, exluding the finance and hr folders in C:\docs.

   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath C:\Folder -FileAge 30 -LogFile H:\log.log -IncludePath "C:\Folder\Docs\","C:\Folder\Users\" -ExcludePath "C:\docs\finance\","C:\docs\hr\"
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath C:\Folder -FileAge 30 -LogFile H:\log.log -IncludePath "C:\Folder\Docs\","C:\Folder\Users\" -ExcludePath "C:\docs\finance\","C:\docs\hr\" -ExcludeDate 'Week,Fri,10','Year,last,-1','LimitYear,1950'
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath C:\Folder -FileAge 60 -LogFile H:\log.log -IncludePath .*images.* -RegExPath

      Description:
      Delete files older than 60 days and only delete files that contain 'images' in their path name.

   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath C:\Folder -FileAge 45 -LastAccessTime -LogFile H:\log.log

      Description:
      Delete files that have not been access for more than 45 days.

   .EXAMPLE
      PowerShell.exe Remove-OldFiles.ps1 -FolderPath 'H:\admin_user' -FileAge 10 -LogFile C:\log -AutoLog
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath H:\SQL\BackUp -FileAge 10 -LogFile C:\log -AutoLog -KeepFile 5
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath C:\docs -FileAge 30 -logfile h:\log.log -CreateTime -NoFolder

      Description:
      Deletes all files that were created 30 days ago or before in the C:\docs folder. No folders are deleted.

   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath C:\docs -FileAge 30 -logfile h:\log.log -CreateTime -CleanFolders
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath C:\docs -FileAge 30 -logfile h:\log.log -CompareCreateTimeLastModified

      Description:
      Deletes all files that were not created or modified 30 days ago or before in the C:\docs folder.

   .EXAMPLE
      .\Remove-OldFiles.ps1 -folderpath c:\users\someuser\desktop -fileage 10 -log c:\log.txt -autolog -verboselog -IncludeFileExtension '.xls*','.doc*'
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -folderpath c:\users\someuser\desktop -fileage 10 -log c:\log.txt -autolog -verboselog -ExcludeFileExtension .xls
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath C:\docs -FileAge 30 -LogFile h:\log.log -ExcludeDate 'Week,Thu,4','Month,last,-1','Quarter,first,6','Year,last,10','LimitYear,2012','Date,1990-12-31','Date,1995-5-31'
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath H:\scripts -FileAge 100 -ListOnly -LogFile H:\log.log -ExcludeDate 'DateRange,2005-05-16,8'
   ...
   .EXAMPLE
      .\Remove-OldFiles.ps1 -FolderPath C:\docs -ListOnly -FileAge 30 -LogFile h:\log.log -ExcludeDate 'Month,15,5','Month,16,5' -EmailTo someuser@corp.co -EmailFrom someuser@corp.co -EmailSmtpServer smtp.corp.co

      Description:
      Deletes all files that were created 30 days ago or before in the C:\docs folder. With the exclusion of files last modified/created on either the 15th or 16th of the last five months. After completion of the script the log file will be emailed to someuser@corp.co via the smtp.corp.co smtp server.
#>
...
(
   ...
   #Option email parameter, allows for setting a different subject for the email containing the log file. The default formatting of the subject is 'Remove-OldFiles.ps1 started at: $StartTime FolderPath: $FolderPath'
   [string]   $EmailSubject,
   ...
   #Automatically generates filename at path specified in -logfile. If a filename is specified in the LogFile parameter and the AutoLog parameter is used only the path specified in LogFile is used. The file name is created with the following naming convention: "Autolog_<FolderPath><dd-MM-yyyy_HHmm.ss>.log"
   [switch]   $AutoLog,
...
function ConvertTo-DateObject
{
   ...
   if ($DayOfQuarter -ne $null)
   {
      ...
      } while ($(if ($QuarterRepeat -eq -1) { $LimitYear -le $(if ($CheckDate) { $CheckDate.Year } else { 9999 }) } else { $j -lt $QuarterLoopCount }))
   }
   ...
   if ($DayOfYear -ne $null)
   {
      ...
      do
      {
         ...
         $YearInt--
      } while ($(if ($DayOfYearRepeat -eq -1) { $LimitYear -le $(if ($CheckDate) { $CheckDate.Year } else { 9999 }) } else { $j -lt $YearLoopCount }))
   }
...
if ($CompareCreateTimeLastModified)
{
   $Switches += "`r`n`t`t-CompareCreateTimeLastModified"
}
...
if ($EmailTo -or $EmailFrom -or $EmailSmtpServer)
{
   ...
   else
   {
      ...
      {
         $EmailSplat.Subject = "Remove-OldFiles.ps1 started at: $StartTime FolderPath: $FolderPath"
      }
...
Write-Output ('-' * 79)
Write-Output "  Remove-OldFiles`t::`tScript to delete old files from folders"
Write-Output ('-' * 79)
...
"  Remove-OldFiles`t::`tScript to delete old files from folders" | Add-Content -LiteralPath $LogFile
('-' * 79) | Add-Content -LiteralPath $LogFile
      ...
      if (-not $ListOnly)
      {
         ...
         if ($VerboseLog)
         {
            switch ($true)
            {
               { $CreateTime } 
               { 
                  "`tDELETED FILE`t$($FileList[$j].CreationTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" | Add-Content -LiteralPath $LogFile 
               }
               { $LastAccessTime } 
               { 
                  "`tDELETED FILE`t$($FileList[$j].LastAccessTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" | Add-Content -LiteralPath $LogFile 
               }
               { $CompareCreateTimeLastModified } 
               { 
                  "`tDELETED FILE`t$($FileList[$j].CustomTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" | Add-Content -LiteralPath $LogFile 
               }
               Default
               {
                  "`tDELETED FILE`t$($FileList[$j].LastWriteTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" | Add-Content -LiteralPath $LogFile
               }
            }
         }
      }
   ...
   if ($ListOnly)
   {
      if ($VerboseLog)
      {
         switch ($true)
         {
            { $CreateTime } 
            { 
               "`tLISTONLY`t$($FileList[$j].CreationTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" | Add-Content -LiteralPath $LogFile 
            }
            { $LastAccessTime } 
            { 
               "`tLISTONLY`t$($FileList[$j].LastAccessTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" | Add-Content -LiteralPath $LogFile 
            }
            { $CompareCreateTimeLastModified } 
            { 
               "`tLISTONLY`t$($FileList[$j].CustomTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" | Add-Content -LiteralPath $LogFile 
            }
            Default 
            { 
               "`tLISTONLY`t$($FileList[$j].LastWriteTime.ToString('yyyy-MM-dd hh:mm:ss'))`t$($FileList[$j].Length.ToString().PadLeft(15))`t$tempfile" | Add-Content -LiteralPath $LogFile 
            }
         }
      }
      else
